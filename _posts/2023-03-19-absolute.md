...
When following [the guide](https://icyguider.github.io/2022/05/19/NoFix-LPE-Using-KrbRelay-With-Shadow-Credentials.html){:target="_blank"} on the github there is one major difference we have to account for. The guide executes `KrbRelay` in an interactive session (i.e. through RDP GUI). However we are in a `WinRM` shell. `WinRM` runs from [session 0](https://kb.firedaemon.com/support/solutions/articles/4000086228-microsoft-windows-session-0-isolation-and-interactive-services-detection){:target="_blank"} meaning processes run in a completely separate space from interactive sessions when we login interactively, and for this reason causes `KrbRelay` to not work. A way round this is to use `RunasCS` which bypasses this limitation and creates a false interactive session with the credentials we provide it, allowing `KrbRelay` to work! 
...
Googling the error we see the logon type we tried has not been granted to our user. By default `RunasCS` tries `Logon Type 8` (Network Cleartext). 
...
Lets try `Logon Type 9` by specifying `-l 9` and this produces a different error relating to a `COMException`. This time it looks like our `CLSID` we specified caused the error. 
