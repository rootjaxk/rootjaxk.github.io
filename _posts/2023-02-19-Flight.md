...
We see the account trying to authenticate is `flight\G0$`. Machine accounts have `120` character passwords so the hash wont crack like we've seen before. However, as our current appool user we can extract the `TGT` of the DC account giving everything we need to impersonate the DC, simulate its the replication behaviour and extract all credential matter from the `NTDS.DIT`! First we need to extract the `TGT` of the `g0$` account from memory which we can do with the `tgtdeleg` flag from [Rubeus](https://github.com/GhostPack/Rubeus).
